...
export const fetchSellerOrders = createAsyncThunk<Order[] , string>(
    ...
    async(jwt , {rejectWithValue}) => {
        try {
            ...
            console.log("seller product fetched successfully" , response.data);
            return response.data
        } catch(error:any) {
            return rejectWithValue(
                error.response || "Failed to fetch all the orders"
            )
        }
    }
)
...
interface SellerOrderStatus {
    orders:Order[];
    loading:boolean;
    error:string | null;
}

const initialState:SellerOrderStatus={
    orders:[],
    loading:false,
    error:null
}
